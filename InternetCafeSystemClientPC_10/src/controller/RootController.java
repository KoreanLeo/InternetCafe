		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				try {
					...
					pw.println("10번자리:"+measage1Field.getText()+"\n");
					pw.flush();
				} catch (IOException e) {
				...
				if(cardmoney.selectedProperty().getValue().equals(true)) {
					for(int i=0;i<OB.size();i++) {
					    ...
					    String date = fm2.format(new Date());
					    	fi2=new Finalorder2("10",logtxtID.getText(), menu, amount, fprice1,cardmoney.getUserData().toString() , moneyWay.getSelectionModel().getSelectedItem().toString(),date,ordermesage.getText());
					...
					for(int i=0;i<OB.size();i++) {
						...
						SimpleDateFormat fm2= new SimpleDateFormat(
					            "yyyy년MM월dd일HH시mm분ss초");
					    String date = fm2.format(new Date());
					    	fi2=new Finalorder2("10",logtxtID.getText(), menu, amount, fprice1, realmoney.getUserData().toString(), moneyWay.getSelectionModel().getSelectedItem().toString(),date,ordermesage.getText());
							int count = MemberDAO.insertorderDate(fi2);
					}
					OB.clear();
					moneyWay.getSelectionModel().clearSelection();
